...
function ControlPro() {
  const { user , logoutUser, uploadImage, addToDabase } = useAuth()
  const navigate = useNavigate();
  const [productTitle, setProductTitle ] = useState('')
  const [ productDes, setProductDes] = useState('')
  const [ price, setPrice ] = useState("")
  const [ catagory, setCatagory ] = useState('Mobile Cover')
  ...
  const handleSubmit = async(e) => {
    ...
    if (user) {
      try {
        ...
        await addToDabase({ productTitle, productDes, price, catagory, imagekey: fileId });
      } catch (error) {
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      ...
      id="grid-first-name" type="text" placeholder="Mobile Cover"/>
    ...
    <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
      ...
      <div class="relative">
        ...
        id="grid-state">
          ...
          <option>Custome Cover </option>
          <option>Gift Product</option>
        </select>
